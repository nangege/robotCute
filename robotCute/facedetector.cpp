...
{
    if(faceClassifier.load(FACE_CASCADE))
    {
        FaceEnable = true;
    }
    else
    ...
    if(eyesClassifier.load(EYE_CASCADE))
    {
        EyeEnable = true;
    }
    else
    ...
    if(glassEyesClassifier.load(EYE_TREE_CASCADE))
    {
        GlassEyeEnable = true;
    }
    else
    ...
    if(noseClassifier.load(NOSE_CASCADE))
    {
        NoseEnable = true;
    }
    else
    ...
    if(mouthCalssifier.load(MOUSE_CASCADE))
    {
        MouthEnable = true;
    }
    else
...
{
    widthScale = grayImage.cols/WIDTH;
    ...
    //imshow("preImage",grayImage);
    //moveWindow("preImage",640,0);
}
...
void faceDetector::detectRightEye()
{
    /*if(EyeEnable)
    {
        if(isFaceDetected())
        {
            Rect eyeRect = Rect(FaceRIO.width/2,0,FaceRIO.width/2,FaceRIO.height/2);
            Mat eyesMat = faceMat(eyeRect);
            detectObject(eyesClassifier,eyesMat,rightEyes);

            if(rightEyes.size() == 0)
            {
                if(GlassEyeEnable)
                {
                    detectObject(glassEyesClassifier,eyesMat,rightEyes);
                }
            }
        }
...
void faceDetector::detectMouth()
{
    if(MouthEnable)
    {
        if(isFaceDetected())
        {
            ...
            detectObject(mouthCalssifier,mouthMat,mouth);
        }
